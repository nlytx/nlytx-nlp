...
import cc.factorie.nlp.coref.WithinDocCoref
...
class Document extends DocumentSubstring with Attr with UniqueId with Serializable {
  ...
  //TODO Disabled owplString as it references DocumentAnnotationPipeline, but could probably be re-implemented for library use
